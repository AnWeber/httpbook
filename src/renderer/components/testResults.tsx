  ...
  <div class="testresults">
    <div class="testtitle">
      <h4 class="testtitle__cell">
        <span>{testResults.filter(obj => obj.status === 'SUCCESS').length}</span> Passing Tests
      </h4>
      <h4 class="testtitle__cell">
        <span className={testResults.filter(obj => obj.status === 'FAILED').length > 0 ? 'is_error' : ''}>
          {testResults.filter(obj => obj.status === 'FAILED').length}
        </span>{' '}
        Failing Tests
      </h4>
    </div>
...
const TestResultLine: FunctionComponent<{ testResult: TestResult }> = ({ testResult }) => (
  <article className={getStatusCssClass(testResult.status)}>
    <TestResultStatusIcon status={testResult.status}></TestResultStatusIcon>
    <div>
...
function getStatusCssClass(status: TestResultStatus) {
  if (status === 'SUCCESS') {
    return 'testline testline--success';
  }
  if (status === 'SKIPPED') {
    return 'testline testline--skipped';
  }
  if (status === 'FAILED') {
    return 'testline testline--failed';
  }
  return 'testline testline--error';
}

const TestResultStatusIcon: FunctionComponent<{ status: TestResultStatus }> = ({ status }) => {
  if (status === 'SUCCESS') {
    return (
